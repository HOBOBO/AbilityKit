...
/// <summary>
/// acceptance timeline 的 dotnet 执行：按 atMs 排序推进 sim 时钟；wait/tick → tick；环境动词 → <see cref="LiveSimSetupActionExecutor"/>；
/// 技能动词（press/release/hold/cancel/cast_skill/skill_input）→ 富 <c>SkillInputEvent</c> → <c>PlayerInputCommand</c> →
/// <c>IMobaInputCoordinator.TrySubmit</c>。断言失败抛 <see cref="InvalidOperationException"/>。
/// </summary>
public sealed class LiveSimTimelineRunner
{
        ...
        MobaAcceptanceVector3Expectation position, MobaAcceptanceVector3Expectation direction, string context)
    {
        ...
        {
            if (!_setup.TryGetActorId(targetAlias, out resolvedTargetActorId))
                throw new InvalidOperationException($"Target actor alias missing: {targetAlias}");
        }

        // press-aim 归一化：纯方向 press 归一为无 aim。
        ...
        if (!services.TryResolve<IMobaInputCoordinator>(out var input) || input == null)
            throw new InvalidOperationException("IMobaInputCoordinator must be resolvable from the console world.");
        ...
        if (!result.Succeeded)
            throw new InvalidOperationException($"Scenario skill input rejected. {context}, actorAlias={actorAlias}, slot={slot}, phase={phase}, castFrame={castFrame.Value}, result={result}");
        if (result.HandledCount <= 0)
            throw new InvalidOperationException($"Scenario skill input not handled. {context}, result={result}");
    }
