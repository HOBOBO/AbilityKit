    ...
    internal sealed class GameplayTagTreeView : TreeView
    {
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            for (int i = 0; i < entries.Count; i++)
            {
                var e = entries[i];
                if (e == null) continue;
                if (GameplayTagValidator.TryNormalize(e.Name, out var n))
                {
                    normalized.Add(n);
                    _byName[n] = e;
                }
        ...
        protected override void RowGUI(RowGUIArgs args)
        {
            ...
            for (int i = 0; i < args.GetNumVisibleColumns(); i++)
            {
                ...
                {
                    EditorGUI.LabelField(rect, GetCategory(item));
                }
            }
        }
        ...
        private string GetCategory(TreeViewItem item)
        {
            var full = GetFullName(item);
            if (string.IsNullOrEmpty(full)) return string.Empty;
            if (_byName.TryGetValue(full, out var e) && e != null)
            {
                return e.Category ?? string.Empty;
            }
            var dotIndex = full.IndexOf('.');
            return dotIndex > 0 ? full.Substring(0, dotIndex) : string.Empty;
        }
        ...
        protected override void ContextClickedItem(int id)
        {
            ...
            if (!string.IsNullOrEmpty(full))
            {
                ...
                menu.AddItem(new GUIContent("Edit Category"), false, () => EditCategory(full));
                menu.AddItem(new GUIContent("Copy Tag Name"), false, () => CopyTagName(full));
            ...
            else
            {
                ...
                menu.AddDisabledItem(new GUIContent("Edit Category"));
                menu.AddDisabledItem(new GUIContent("Copy Tag Name"));
        ...
        private void AddChild(string parentFull)
        {
            ...
            GameplayTagTextPrompt.Open(
                ...
                onOk: segment =>
                {
                    ...
                    if (!GameplayTagValidator.IsValidSegment(segment))
                    {
                    ...
                    if (!GameplayTagValidator.TryNormalize(full, out var normalized))
                    {
        ...
        private void Rename(string full)
        {
            ...
            GameplayTagTextPrompt.Open(
                ...
                onOk: newSeg =>
                {
                    ...
                    newSeg = newSeg.Trim();
                    if (!GameplayTagValidator.IsValidSegment(newSeg))
                    {
                        EditorUtility.DisplayDialog("Gameplay Tags", "Invalid segment. No whitespace, no '.'", "Close");
                        return;
                    }
        ...
        private void EditCategory(string full)
        {
            if (string.IsNullOrEmpty(full)) return;
            var e = _db.GetOrCreate(full);
            if (e == null) return;
            ...
            GameplayTagTextPrompt.Open(
                title: "Edit Category",
                label: "Category for: " + full,
                defaultText: e.Category ?? string.Empty,
                onOk: category =>
                {
                    Undo.RecordObject(_db, "Edit Tag Category");
                    e.Category = category ?? string.Empty;
                    OnDatabaseChanged?.Invoke();
                }
            );
        }

        private void CopyTagName(string full)
        {
            if (string.IsNullOrEmpty(full)) return;
            EditorGUIUtility.systemCopyBuffer = full;
            Debug.Log($"[GameplayTag] Copied: {full}");
        }
        ...
        private string MoveUnder(string fromFull, string newParentFull)
        {
            ...
            if (!GameplayTagValidator.IsValidSegment(seg)) return null;
            ...
            if (!GameplayTagValidator.TryNormalize(newFull, out var normalized)) return null;
            return normalized;
        }
        ...
        protected override bool DoesItemMatchSearch(TreeViewItem item, string search)
        {
            ...
            return (!string.IsNullOrEmpty(full) && full.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0)
                ...
                || (!string.IsNullOrEmpty(category) && category.IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0);
        }
    }
}
