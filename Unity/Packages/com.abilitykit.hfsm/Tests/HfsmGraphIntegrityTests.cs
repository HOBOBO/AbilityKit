...
{
    public sealed class HfsmGraphIntegrityTests
    {
        ...
        private sealed class EvaluationContext : IHfsmEvaluationContext
        {
        ...
        public sealed class ArbitraryConfigCondition : HfsmTransitionCondition
        {
            public string Alias;
            public int Count;
            public bool Enabled;
            ...
            public override bool Evaluate(IHfsmEvaluationContext context) => Enabled;
            public override HfsmTransitionCondition Clone() => new ArbitraryConfigCondition
            {
                Alias = Alias,
                Count = Count,
                Enabled = Enabled,
        ...
        {
            var node = new HfsmStateNode("Idle")
            {
                ParentStateMachineId = "parent",
                isDefault = true
            };

            var restored = JsonUtility.FromJson<HfsmStateNode>(JsonUtility.ToJson(node));
        ...
        public void GraphCloneRemapsAllInternalReferencesAndEditorMetadata()
        {
            ...
            HfsmGraphAsset clone = null;
            try
            {
                clone = graph.Clone();
                var clonedRoot = clone.Nodes.OfType<HfsmStateMachineNode>().Single(node => node.DisplayName == root.DisplayName);
                var clonedNested = clone.Nodes.OfType<HfsmStateMachineNode>().Single(node => node.DisplayName == nested.DisplayName);
                var clonedIdle = clone.Nodes.OfType<HfsmStateNode>().Single(node => node.DisplayName == idle.DisplayName);
                var clonedAttack = clone.Nodes.OfType<HfsmStateNode>().Single(node => node.DisplayName == attack.DisplayName);
        ...
        {
            var edge = new HfsmTransitionEdge();
            edge.AddCondition(new HfsmParameterCondition { ParameterName = "first" });
            _ = edge.Conditions;

            var replacement = new HfsmTransitionEdge();
            replacement.AddCondition(new HfsmParameterCondition { ParameterName = "second" });
            edge.ConditionConfigJson = replacement.ConditionConfigJson;

            var condition = (HfsmParameterCondition)edge.Conditions.Single();
        ...
        {
            var serialized = new HfsmTransitionEdge();
            serialized.AddCondition(new HfsmParameterCondition { ParameterName = "existing" });

            var restored = new HfsmTransitionEdge
            {
                ConditionConfigJson = serialized.ConditionConfigJson
            };
            restored.AddCondition(new HfsmParameterCondition { ParameterName = "added" });

            Assert.That(
                restored.Conditions.Cast<HfsmParameterCondition>().Select(condition => condition.ParameterName),
        ...
        {
            HfsmConditionRegistry.Register<ArbitraryConfigCondition>();
            var edge = new HfsmTransitionEdge();
            edge.AddCondition(new HfsmTimeElapsedCondition
            {
            ...
            var restored = new HfsmTransitionEdge { ConditionConfigJson = edge.ConditionConfigJson };

            var elapsed = (HfsmTimeElapsedCondition)restored.Conditions[0];
        ...
        public void CompiledConditionsGateNormalTransitions()
        {
            ...
            try
            {
                ...
                edge.AddCondition(new HfsmParameterCondition
                {
                    ParameterName = "allow",
                    ParameterType = HfsmParameterType.Bool,
                    BoolValue = true
                });
        ...
        {
            var store = new HfsmParameterStore();
            store.LoadDefaults(new[]
            {
                new ParameterProgram("enabled", HfsmParameterType.Bool, true),
                new ParameterProgram("speed", HfsmParameterType.Float, 1.5f),
                new ParameterProgram("count", HfsmParameterType.Int, 3),
                new ParameterProgram("pulse", HfsmParameterType.Trigger, false)
            });
        ...
        private static HfsmGraphAsset CreateFlatGraph(
            out HfsmStateMachineNode root,
