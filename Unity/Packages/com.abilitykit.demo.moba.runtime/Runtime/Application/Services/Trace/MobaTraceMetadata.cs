...
namespace AbilityKit.Demo.Moba.Services
{
    ...
    public sealed class MobaTraceMetadata : TraceMetadata
    {
        ...
        {
            return $"{TraceKind}(root={RootId}, config={ConfigId}, trigger={TriggerId}, skill={SkillId}, castFlow={CastFlowId}, phase={PhaseId}, source={SourceActorId}, target={TargetActorId}, origin={OriginSource}, targetOrigin={OriginTarget})";
        }

        public bool IsEmpty => RootId <= 0 && SourceActorId <= 0 && TargetActorId <= 0 &&
                               ConfigId <= 0 && TriggerId <= 0 && SkillId <= 0 && CastFlowId <= 0 &&
