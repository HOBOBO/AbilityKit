...
namespace AbilityKit.Ability.Share.Impl.Moba.Services
{
    public sealed class MobaOngoingEffectService : IService
    {
        ...
        public IRunningAction Start(int ongoingEffectId, int sourceActorId, int targetActorId, long ownerKey)
        {
            ...
            // keep historical behavior: apply immediately when started.
                    ...
                    if (_config.TryGetOngoingEffect(rt.OngoingEffectId, out var cfg) && cfg != null)
                    {
                        if (cfg.OnRemoveEffectId > 0)
                        {
                            ...
                            ctx.Initialize(
                                ...
                                worldServices: null,
                                eventBus: null);
                            _effectExec.Execute(cfg.OnRemoveEffectId, ctx, AbilityKit.Ability.Impl.Moba.EffectExecuteMode.InternalOnly);
                        }
                    }
                }
                catch (Exception ex)
