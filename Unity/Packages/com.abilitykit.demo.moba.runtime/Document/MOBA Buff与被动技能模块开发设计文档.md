...
### Runtime Context 恢复与移除契约

- Trace 只负责来源链；Buff Runtime Context 的实时值与最终快照由逻辑服务维护，编辑器只读查询，不参与生命周期处理。
- 状态恢复显式重建持久化的 Runtime Context ID/Version，不重新分配 ID，不覆盖已有实体。重复 ID、无效版本、无关上下文冲突与错误绑定归属必须在清理现有 Buff 前拒绝。
- 历史载荷中的 ID=0/Version=0 保持为无 Runtime Context 身份；后续正常绑定才分配新身份。恢复实体只推进分配游标，不回退游标。
- 移除顺序为停止持续行为、冻结最终快照、解绑实时 provider、销毁 Context 实体、发布移除事件与执行移除效果。此阶段保留 ID/Version，查询落到快照；最终清理与回池时清零引用。
- Trace 来源 ID 缺失不应阻止独立 Runtime Context 清理。重复销毁不得覆盖已冻结的最终快照；结束回调异常后仍需完成清理。
