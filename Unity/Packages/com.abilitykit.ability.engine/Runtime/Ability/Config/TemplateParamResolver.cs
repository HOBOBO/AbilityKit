...
namespace AbilityKit.Ability.Configs
{
    ...
    {
        public static void ApplyToObject(object target, IReadOnlyDictionary<string, ArgRuntimeEntryCore> bindings)
        {
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                if (ft == typeof(ArgRuntimeEntryCore))
                {
                    var e = (ArgRuntimeEntryCore)f.GetValue(target);
                    if (e == null) continue;
                    if (e.Kind != ArgValueKind.String) continue;
                    var strVal = e.Value as string;
                    if (!TryParseParamRef(strVal, out var name)) continue;
                    if (!bindings.TryGetValue(name, out var bound) || bound == null) continue;
