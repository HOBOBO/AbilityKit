...
namespace Emilia.Node.Editor
{
    ...
    public class GraphSelectionDragger : Dragger
    {
        ...
        protected override void RegisterCallbacksOnTarget()
        {
            ...
            if (selectionContainer == null) throw new InvalidOperationException("Manipulator can only be added to a control that supports selection");
        ...
        protected void OnMouseCaptureOutEvent(MouseCaptureOutEvent e)
        {
            if (m_Active)
            {
                ...
                {
                    if (m_PrevDropTarget.CanAcceptDrop(m_GraphView.selection)) this.m_PrevDropTarget.DragExited();
                }
        ...
        protected new void OnMouseDown(MouseDownEvent e)
        {
            ...
            if (CanStartManipulation(e))
            {
                ...
                if (! clickedElement.IsMovable() || ! clickedElement.HitTest(clickedElement.WorldToLocal(e.mousePosition))) return;
                ...
                foreach (GraphElement ce in elementsToMove)
                {
                    ...
                    if (ce.parent is StackNode)
                    {
                        ...
                        if (stackNode.IsSelected(m_GraphView)) continue;
                    }
        ...
        protected void ComputeSnappedRect(ref Rect selectedElementProposedGeom, float scale)
        {
            ...
            geometryInContentViewContainerSpace = m_Snapper.GetSnappedRect_Internals(geometryInContentViewContainerSpace, scale);
        ...
        protected new void OnMouseMove(MouseMoveEvent e)
        {
            ...
            if (snapEnabled && ! m_ShiftClicked) ComputeSnappedRect(ref selectedElementGeom, this.m_XScale);

            if (snapEnabled && m_ShiftClicked) this.m_Snapper.ClearSnapLines_Internals();
            ...
            foreach (KeyValuePair<GraphElement, OriginalPos> v in m_OriginalPos)
            {
                ...
                if (! v.Value.dragStarted)
                {
                    ...
                    if (stackParent != null) stackParent.OnStartDragging(ce);
                    ...
                    if (groupElementsDraggedOut != null)
                    {
                        ...
                        if (groupParent != null)
                        {
                            if (! groupElementsDraggedOut.ContainsKey(groupParent)) groupElementsDraggedOut[groupParent] = new List<GraphElement>();
            ...
            if (groupElementsDraggedOut != null)
            {
                foreach (KeyValuePair<Group, List<GraphElement>> kvp in groupElementsDraggedOut) kvp.Key.OnStartDragging_Internals(e, kvp.Value);
            }
            ...
            if (CanStopManipulation(evt))
            {
                if (m_Active)
                {
                    if (m_Dragging)
                    {
                        ...
                        {
                            if (grouping.Key != null && m_GraphView.elementsRemovedFromStackNode != null) this.m_GraphView.elementsRemovedFromStackNode(grouping.Key, grouping);

                            foreach (GraphElement ge in grouping) ge.UpdatePresenterPosition();
        ...
        protected void OnKeyDown(KeyDownEvent e)
        {
            ...
            foreach (KeyValuePair<GraphElement, OriginalPos> v in m_OriginalPos)
            {
                ...
                else
                {
                    ...
                    {
                        if (! groupsElementsToReset.ContainsKey(originalPos.scope)) groupsElementsToReset[originalPos.scope] = new List<GraphElement>();
            ...
            foreach (KeyValuePair<Scope, List<GraphElement>> toReset in groupsElementsToReset) toReset.Key.AddElements(toReset.Value);
