...
namespace Emilia.Node.Editor
{
    ...
    public class EditorEdgeManipulator : MouseManipulator
    {
        ...
        protected virtual void OnMouseMove(MouseMoveEvent evt)
        {
            ...
            bool alreadyDetached = this.m_DetachedPort != null;
                ...
                if (m_DetachedPort.allowMultiDrag)
                {
                    foreach (var edge in m_DetachedPort.connections)
                    {
                        if (edge != m_Edge && edge.IsSelected(gv))
                        {
                            ...
                            if (m_DetachedPort == edge.input) edge.input = null;
                            else edge.output = null;
                    ...
                    foreach (var edge in m_AdditionalEdges) m_DetachedPort.Disconnect(edge);
                }
        ...
        {
            bool shouldStop = CanStopManipulation(evt) || (m_Active && evt.button == (int)MouseButton.RightMouse);

            if (shouldStop)
            {
                target.ReleaseMouse();

                // If right-click during active drag, cancel the operation
        ...
        protected virtual void RestoreDetachedPort()
        {
            if (m_DetachedFromInputPort)
            {
