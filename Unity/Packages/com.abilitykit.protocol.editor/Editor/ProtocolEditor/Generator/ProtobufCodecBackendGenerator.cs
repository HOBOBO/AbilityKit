...
{
    internal sealed class ProtobufCodecBackendGenerator : ICodecBackendGenerator
    {
        public ProtocolDefinition.CodecBackend Backend => ProtocolDefinition.CodecBackend.Protobuf;
        ...
        public void GenerateImplementationStubs(ProtocolDefinition def, string outputFolder, string @namespace)
        {
            ...
            var className = $"SnapshotProtobuf_{SanitizeIdentifier(def.RegistryId)}";
            var fileName = $"SnapshotProtobuf.{SanitizeIdentifier(def.RegistryId)}.cs";
            ...
            foreach (var msg in def.Messages)
            {
                ...
                if (msg.Backend != ProtocolDefinition.CodecBackend.Protobuf) continue;
                ...
                switch (msg.Channel)
                {
                    case ProtocolDefinition.ChannelKind.SnapshotDecoder:
                        ...
                        sb.AppendLine("        {");
                        sb.AppendLine("            throw new System.NotSupportedException(\"Protobuf snapshot decode not implemented for " + methodName + " (" + payloadType + "); fill in the decoder.\");");
                        sb.AppendLine("        }");
                        sb.AppendLine();
                        break;
                    ...
                    case ProtocolDefinition.ChannelKind.SnapshotCmdHandler:
                        ...
                        sb.AppendLine("        {");
                        sb.AppendLine("            throw new System.NotSupportedException(\"Protobuf snapshot command handler not implemented for " + methodName + ".\");");
                        sb.AppendLine("        }");
                        sb.AppendLine();
                        break;
                    ...
                    case ProtocolDefinition.ChannelKind.SnapshotPipelineStage:
                        ...
                        sb.AppendLine("        {");
                        sb.AppendLine("            throw new System.NotSupportedException(\"Protobuf snapshot pipeline stage not implemented for " + methodName + ".\");");
                        sb.AppendLine("        }");
                        sb.AppendLine();
                        break;
                }
