...
namespace AbilityKit.Ability.Host.Examples
{
    public static class LogicWorldServerExample
    {
        ...
        public static LogicWorldServer CreateFrameSyncServerWithOneWorld(WorldId worldId)
        {
            ...
            var options = new LogicWorldServerOptions();
            var modules = new LogicWorldServerModuleHost();
            modules.Add(new ServerFrameTimeModule());
            modules.InstallAll(options);

            var server = new LogicWorldServer(manager, options);
        ...
        public static (LogicWorldServer server, ServerRollbackModule rollback) CreateFrameSyncRollbackServerWithOneWorld(WorldId worldId, int historyFrames, int captureEveryNFrames)
        {
            var registry = new WorldTypeRegistry();
            registry.Register(ExampleWorldType, MinimalWorldFactory.CreateWorld);
            var manager = new WorldManager(new RegistryWorldFactory(registry));
