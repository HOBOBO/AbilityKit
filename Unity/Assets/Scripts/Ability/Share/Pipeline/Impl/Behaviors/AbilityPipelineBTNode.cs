namespace AbilityKit.Ability
{
    ...
    {
        EBTNodeStatus Tick(IAbilityPipelineContext context, float deltaTime);
        void Reset();
    }
    ...
    /// 用于将管线阶段包装成行为树节点
    /// </summary>
    public abstract class AbilityPipelineBTNode : IBTNode
    {
        ...
        /// 每帧执行
        /// </summary>
        public virtual EBTNodeStatus Tick(IAbilityPipelineContext context, float deltaTime)
        {
            if (!_isStarted)
            {
                OnStart(context);
        ...
        /// 节点开始时调用
        /// </summary>
        protected virtual void OnStart(IAbilityPipelineContext context) { }

        /// <summary>
        /// 每帧更新，返回节点状态
        /// </summary>
        protected abstract EBTNodeStatus OnUpdate(IAbilityPipelineContext context, float deltaTime);

        /// <summary>
        /// 节点结束时调用
        /// </summary>
        protected virtual void OnEnd(IAbilityPipelineContext context, bool success) { }
    }
    ...
    /// </summary>
    public class AbilityPhaseBTNode : AbilityPipelineBTNode
    {
        private readonly IAbilityPipelinePhase _phase;
        private IDurationalPhase _durationalPhase;

        public AbilityPhaseBTNode(IAbilityPipelinePhase phase)
        {
            _phase = phase;
            _durationalPhase = phase as IDurationalPhase;
        }

        protected override void OnStart(IAbilityPipelineContext context)
        {
            base.OnStart(context);
            _phase.Execute(context);
        }

        protected override EBTNodeStatus OnUpdate(IAbilityPipelineContext context, float deltaTime)
        {
            // 非持续性阶段，执行完立即成功
    ...
    /// </summary>
    public class BTSequenceNode : AbilityPipelineBTNode
    {
        private readonly IBTNode[] _children;
        private int _currentIndex = 0;

        public BTSequenceNode(params IBTNode[] children)
        {
            _children = children;
        }
    ...
    /// </summary>
    public class BTSelectorNode : AbilityPipelineBTNode
    {
        private readonly IBTNode[] _children;
        private int _currentIndex = 0;

        public BTSelectorNode(params IBTNode[] children)
        {
            _children = children;
        }
    ...
    /// 同时执行所有子节点
    /// </summary>
    public class BTParallelNode : AbilityPipelineBTNode
    {
        /// <summary>
        /// 并行策略
        /// </summary>
        ...
        public BTParallelNode(ParallelPolicy policy, params IBTNode[] children)
        {
            _children = children;
