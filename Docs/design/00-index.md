...
| [03-快速开始](01-OverviewAndGettingStarted/03-QuickStart.md) | 入门路径 | §2.1 第 0 步跑示例目录（37 条分级示例，无需配置与服务器）与五个 Starter、golden 基线校验；dotnet 构建、Console Demo CLI、SYNC_MODE、启动阶段、自动测试、测试入口和分阶段阅读路径 |
...
| [05-示例体系 Unity 化：Sample Catalog 进包与 Unity 宿主设计](01-OverviewAndGettingStarted/05-SampleCatalogUnityHosts.md) | 演进计划（**未实施**） | 新增单个包 `com.abilitykit.samples`（`Runtime/` 纯逻辑 + `Editor/` 宿主），把既有 27 文件零依赖契约层与 37 条示例（18 分类）搬进 `Unity/Packages` 并补 Unity 宿主（CLI/Web 之外的第三个宿主）；净新增包数 **+1**；含 7 类移植阻塞实测清单与发布工具链约束（§6.4，不阻塞实施）；附 `world.di` 补 `IWorldTickDriver`+`DefaultWorld` 的独立可选项 |
...
| [10-通用计算加速后端与 Unity Jobs 扩展框架](13-FrameworkCore/10-ComputeAccelerationBackends.md) | 跨运行时计算加速 | 纯 C# kernel/显式后端链、Unity 原生缓冲与 Job DAG、Burst AOT 边界、校验回退、motion/dataflow 迁移方式和性能准入 |
...
| 2026-09-10 | 2.95 | **落地设计文档的阶段 0**（首批实施，非仅 Markdown）：① 新增 `tools/check_sample_baseline.ps1` 与 `tools/samples/sample-baseline.txt`（37 条示例输出 + manifest 校验，75KB，无 BOM），脚本支持 `-Update` 刷新、逐章节差异归因、`<OUTPUT_DIR>` 与已知墙钟依赖的 `<unstable>` 掩码；② 接入 `foundation-units` 门禁（新增 `Build sample catalog host` 与 `Sample catalog golden baseline` 两步，实测 17.9s + 4.7s）；③ `03-QuickStart.md` v3.1 补可见性缺口——新增 §2.1「第 0 步：跑示例目录」（此前该文档 `grep` Starter 与 `AbilityKit.Samples` 均零命中），§3.1 入口清单与 §4.5 证据补示例目录条目。过程中**发现框架缺陷**：`world.statesync` 的 `StateManager.CaptureState` 给 `WorldStateSnapshot.Timestamp` 打墙钟，而 `StateHashComputer.Compute` 把 Timestamp 计入哈希、`ComputeBinaryDiff` 又是位置敏感的字节差异，导致捕获态哈希与增量差异均依赖墙钟（实测 20 次运行中 `IncrementalDiffBytes` 18×56、1×61）；基线以掩码+警告方式记录该症状，缺陷本身待修 |
| 2026-09-10 | 2.96 | 落地设计文档**阶段 2 的已完成部分**（原地清理移植阻塞，顺序从"先搬包"调整为"先清阻塞"——核实 Unity Editor 正在运行，先搬包会让运行中的 Editor 立刻编译必然报错的新包）：① 删 `Samples.Logic.csproj` 死 xunit 引用；② 两个 csproj 设 `ImplicitUsings=disable` 使 .NET 构建成为 Unity 编译环境的忠实代理，按编译错误补齐 10 处 using（8 文件 `System.Collections.Generic` + 2 文件 `System.Linq`）；③ **额外**发现并修复注释乱码——189 个文件里 47 个含 UTF-8 被按 GBK 解读的乱码（框架包 5263 文件仅 4 个，问题集中在待迁移示例），新增 `tools/fix_sample_mojibake.ps1` 还原 123 行。**该脚本的安全规则经一次真实事故定型**：初版允许改任意行且只校验往返无损，把字符串字面量 `硬直帧` 改成 `Ӳֱ֡`（GBK 双字节恰也是合法 UTF-8），**由 golden 基线抓到**；修正为"只改注释行 + 还原结果必须含中日韩字符"，并修正 BOM 判定（初版 `File.ReadAllText` 会吞 BOM 导致静默剥离 21 个文件的 BOM，已全部还原、现 21/21 一致）。验证：构建 0 错误 + 基线逐字节不变 |
| 2026-09-10 | 2.97 | 跑通验证中发现并处置 `com.abilitykit.protocol` 两个**既有**缺陷（与本批改动无依赖路径，`Protocol.Tests` 只引用 `AbilityKit.Protocol`；已证明非本批引入）：①**已修** `ProtocolCatalogAdvertisementDecodeOptions.Default => new T()` 在 struct 上绑定隐式无参构造（全零），不调用全默认参数构造函数，导致解码界限为 0、拒绝一切非空载荷——用 .NET 10 单文件实测确认该 C# 语义，修法为默认值提 `const` + 显式传参，测试 3 失败→1 失败；全仓扫同类模式仅此一处受影响（另两处是 class）；②**未修，需拍板** advertisement 往返丢字段（`ResponseId`/`CaptureSampleRate`/`SensitiveFields` 不在映射内），重建远端目录通不过校验致自协商误判不兼容，修复需扩展二进制编码并 bump `FormatVersion`（线格式变更）。门禁其余 9 步全绿；该包 170 测试现 169 通过 |
| 2026-09-10 | 2.98 | 一次性修干净上条的 Protocol 缺陷②并恢复全绿：advertisement 线格式 v1→v2——message 类型补 `ResponseId`/`CaptureSampleRate`/`SensitiveFields` 三字段与映射，编码追加（字符串 + 8 字节 double + uint16 计数 + 字符串表），`FormatVersion` 拆为 `CurrentFormatVersion=2`/`MinimumSupportedFormatVersion=1`（`Encode` 只写 v2，解码接受 v1~v2 且 v1 新字段取默认值），`ProtocolCatalogAdvertisementDecodeOptions` 加 `MaximumSensitiveFieldsPerMessage`（默认 64，加在参数表末尾源兼容），double 读取拒绝 NaN/Infinity。**补 1 个用例** `CatalogAdvertisementCodec_DecodesVersion1PayloadWithDefaultedFields` 手写 v1 字节布局——`Encode` 只产 v2，不复用编码器才能覆盖 v1 分支。结果：`AbilityKit.Protocol.Tests` **171/171**、`foundation-units` 门禁 **10 步全绿（99.0s）**。验证边界：仅 .NET 侧编译测试（Unity Editor 运行中不可起 batchmode），所用 API 与同文件既有用法一致、未引入新语言特性 |
| 2026-09-10 | 2.99 | 示例迁移阻塞清理收尾（阶段 2 主体完成）：**#1** STJ→Newtonsoft——审计发现 5 个 STJ 文件中两个是死代码（`ConfigHelper.cs` 172 行全仓零调用整文件删除；`PipelinePhaseRegistry` 4 个 JSON 方法 86 行零调用删除），其余 3 处转换，内容层 STJ 归零；**#4** File→IResourceProvider——发现 `IResourceProvider` 有两份重复定义（Abstractions 版零实现零消费者、Logic 版才是实现者用的），收敛为一份置于 Abstractions 并补 `TryLoadText`，`FileSystemResourceProvider` 移入宿主工程，`ResourceProviders` 默认改宿主无关的 `NullResourceProvider`（抛 `FileNotFoundException` 保住 `SampleConfig` 回退语义），宿主 `Main` 起始处安装实现，`SampleManifest`/HFSM 示例 I/O 改走提供器（路径拼接保留故日志路径不变），内容层文件 I/O 归零；**#2** 逐文件 `#nullable enable`（186 文件，首行+空行）；**#5 性质修正**——8 个 Console 文件全在 `Ability/` 子树（27 文件 2308 行）这个自包含孤岛（不在 manifest／无 Attribute 标注／外部零引用／内部 Tests.RunAll 从未调用），且 `Console.WriteLine` 在 Unity 可编译故非阻塞；仓库 `SamplesLogicRefactorPlan.md` 规定旧示例须归四类之一而该子树未归类，故未擅自删除亦未改造，待拍板。**验证**：构建 0 错误、golden 基线逐字节不变、BOM 21/21 一致、`foundation-units` 门禁 10 步全绿（79.1s） |
| 2026-09-10 | 3.00 | **阶段 1 完成：`com.abilitykit.samples` 单包建立**。源码权威归位 `Unity/Packages/`：`Runtime/`（186 文件，`Abstractions` 26 / `Ability` 27 / `Infrastructure` 36 / `Samples` 93 / 5 根文件 + manifest）、`Document/`、`README.md`、`package.json`、`Runtime/AbilityKit.Samples.asmdef`；`package.json` 为 cohort 0.1.0 + 24 个 AbilityKit 依赖全部 0.1.0 + `com.unity.nuget.newtonsoft-json 3.2.1`，无 BOM；asmdef 声明 `noEngineReferences:true` + 26 装配引用；两个 .NET csproj 改为投影工程（`Abstractions` 子树不重复包含，避免同批类型进两个装配），工程名/sln/`Samples.Test` 引用均未动。**装配引用推导改用可靠规则**：从 csproj 的 `<Compile Include=.../Packages/<pkg>/<sub>/**>` 反推覆盖该子树的**全部祖先 asmdef**——"包里第一个 asmdef"的启发式会错选子装配（triggering.abstractions）、兄弟变体（hfsm 的 Core vs Unity）或 Editor 装配；并处理两个边界（asmdef 位于包根目录时前缀剥离失效；csproj 可能写反斜杠 + `*.cs`）。**`Ability/` 归类裁定为 Legacy（仅保留参考）**，已登记进 `SamplesLogicRefactorPlan.md` §6.1/§6.3（该文档随源码移入包内）。**验证**：构建 0 错误、**golden 基线逐字节不变**、`audit-versions.js` 报 clean、门禁 10 步全绿（84.3s）；文档与工具中的陈旧 `src/AbilityKit.Samples.*` 路径已同步。**未验证项**：包内 .meta 待运行中的 Unity 导入时生成（无法 batchmode 代劳），Unity 侧编译未实测 |
| 2026-09-11 | 3.01 | **阶段 3.1 完成：Unity 宿主 `Editor/`**（5 文件 / 741 行）。`SamplePackageResourceProvider`（按文件名在包内查 TextAsset——示例拼的是宿主形状路径，Unity 下无意义；找不到返回 false 而不抛，示例走各自回退）、`SampleHostEnvironment`（即时 `ISampleEnvironment`，与 Console 宿主同语义故输出一致）、`SampleCatalogWindow`（左目录树+搜索 / 右指南+运行+结构化输出+next 链，输出用最基础 IMGUI 逐条渲染）、`SamplesEditorModule`+本地化（`[InitializeOnLoad]` + `context.Panels.Register` 挂薄 Hub 面板）。**两处按既有约定取舍**：Hub 面板只做入口不内嵌整目录（面板经 `onGUIHandler` 重绘，内嵌会让 Hub 每次重绘解析 manifest）；**不注册菜单贡献**（窗口自带 `[MenuItem("Window/AbilityKit/...")]`，Hub 的 `EditorMenuItemDiscovery.WindowRoot` 会扫到它，再注册同路径贡献会在 Hub 列表重复）。**验证**：`SampleHostEnvironment` 对真实契约独立编译 0 错误 0 警告；`Editor/*.cs` 全量编译后错误仅 CS0246/CS0234 共 44 处且**全部是 Unity/平台类型缺失**，无语法错误、无一处指向示例包或自有类型；本地化键 3/3 对齐。**未验证**：Unity 真实编译（`EditorGUILayout`/`EditorStyles`/`GUI`/`AssetDatabase` 成员名与重载无法离线校验，是唯一剩余风险面） |
| 2026-09-11 | 3.02 | **Unity 编译抓出并修复我引入的兼容性缺陷**：`ProtocolCatalogAdvertisement` 用了 `BinaryPrimitives.WriteDoubleLittleEndian`/`ReadDoubleLittleEndian`——**.NET 5+ 才加入，Unity 的 netstandard2.1 没有**（CS0117）。`dotnet build` 以 net10.0 为目标所以全绿，掩盖了这条仓库既有规则。改为 `WriteInt64LittleEndian(buffer, BitConverter.DoubleToInt64Bits(v))` / `BitConverter.Int64BitsToDouble(ReadInt64LittleEndian(span))`——**字节完全相同**，Protocol 171/171 仍全绿、golden 基线不变。**根因是系统性缺口**：仓库没有任何 netstandard 目标的工程，`.NET 构建全绿 ≠ Unity 能编译`，而这条规则此前只存在于记忆里、文档中无记录 → 已补进 `CLAUDE.md` 的「已知工程坑」（含等价写法与同类需避开的 API 清单）。同时补强 golden 基线：首次实跑暴露基线**自身约 3% 偶发误报**（`sync/state-diff-apply` 有两行携带同一不稳定字节数，初版只掩码了一行），比对逻辑改为**两段式**——不一致时再捕获一次，第二次 match 判 UNSTABLE 退 0、两次彼此一致判确定性回归退 1、两次彼此也不一致退 1；实测两路径退出码正确且真实变异仍被精确报出。门禁 10 步全绿 |
| 2026-09-11 | 3.03 | Unity 编译报出**第二个**搬迁暴露的 Unity 专属缺陷：`SampleManifestValidator.cs` 用了 `init` 访问器（9 处）+ `SampleRegistry.cs`（3 处），而 netstandard2.1 没有 `System.Runtime.CompilerServices.IsExternalInit` → CS0518。该文件此前只在 .NET-only 工程里，从未被 Unity 编译过，搬迁把它暴露出来。修法照仓库既有模式：新增 `Runtime/IsExternalInitPolyfill.cs`（与 scenario/environment/battleflow 版**逐字节一致**，含 `#if !NET5_0_OR_GREATER` 守卫使 net10.0 镜像跳过、无 CS0436）。**并补做一轮离线预检**（因每次 Unity 往返编译都要用户代劳）：① **单装配探针**——把整个 `Runtime/**` 当一个装配编译（Unity 的形态），24 个框架引用下 **0 错误**，证明 Abstractions 与 Logic 两个子树间无重名类型（.NET 拆成两装配时不会报 CS0101，正是盲区）；② .NET 5+ API 黑名单扫描、`Environment`/`Console` 命名空间段、`UnityEngine` 引用（`noEngineReferences` 应禁止）、`record`/file-scoped namespace —— 全部为空。`CLAUDE.md` 的「已知工程坑」相应扩展为三条：BCL 差异、`init` 需 polyfill、包内源码按单装配校验 |
| 2026-09-11 | 3.04 | Unity 编译报出第三、四个搬迁专属缺陷，均已修：**① CS0656 `Microsoft.CSharp.RuntimeBinder.CSharpArgumentInfo`** —— `SampleConfig.cs` 三处 `provider as dynamic` 需要 `Microsoft.CSharp` 引用，而 `noEngineReferences: true` 的装配不会自动获得它。**没有选择加装配引用，而是删掉 `dynamic` 本身**：provider 本就是强类型的 `JsonConfigProvider`，`as dynamic` 纯属多余，删掉既省一条引用、也去掉 AOT 不友好的运行时绑定调用；基线逐字节不变证明行为未变。**② CS0012 缺 `AbilityKit.Deterministic`** —— 暴露了装配引用推导方法的真实缺口：我取的是 csproj **直接** `ProjectReference`，这对 .NET 够用（引用传递）但**对 Unity 不够**（asmdef 必须点名每一个类型被用到的装配）。改为取**传递闭包**：26 → **29 个装配 / 27 个包**（package.json 共 28 条依赖），新增 `Deterministic`、`Network.Runtime`、`Timer`；闭包含包自身 `AbilityKit.Samples`，已排除（asmdef 不能自引用、package.json 不能自依赖）。`CLAUDE.md` 再补两条（asmdef 引用取闭包、`dynamic` 与 `Microsoft.CSharp`）。验证：构建 0 错误、基线逐字节不变、`audit-versions.js` clean、门禁 10 步全绿（149.4s） |
| 2026-09-11 | 3.05 | Unity 编译报出第五个错误，**这次是本次会话新写的 Editor 代码**：`SamplePackageResourceProvider.cs` 用了 `TextAsset` 却只 `using UnityEditor;`（`TextAsset` 属 `UnityEngine`）→ CS0246。**离线检查法抓不到这类错误**：不提供 Unity 程序集时所有 Unity 类型都报 CS0234/CS0246，分不清"整个 Unity 不可用"与"漏了一个 using"。为此新增 `tools/check_unity_usings.js`——按 Unity 类型名清单核对命名空间是否导入。**工具本身按"现有仓库能在 Unity 编译 → 正确的检查器必须对它报零"这条标准收敛**：初版扫 Runtime 得 498 处、扫 Editor 得 316 处，全是误报，逐一消除（收窄到 Editor 范围、剔除 `Editor`/`Time`/`Transform` 等与仓库自有类型/命名空间重名的通用词、剥离注释与字符串字面量、排除 `TriggerValueType.Vector3` 这类成员访问、跳过文件内声明的同名成员如 `public string Color;`）。终态：全仓 **581 个 Editor 文件报零**，且反向验证有效（故意移除 `using UnityEngine;` 立刻报出文件与类型）。已挂进 `CLAUDE.md` 作为动手前的自查入口 |
| 2026-09-11 | 3.06 | **Unity 编译通过（用户实测确认）**——`Runtime` 186 文件 + `Editor` 5 文件全部编译通过，同时验证了此前无法离线校验的那一面（`EditorGUILayout`/`EditorStyles`/`GUI`/`AssetDatabase` 的成员名与重载）。`.meta` 已由 Unity 导入生成：192 个 `.cs.meta` + 52 个目录 meta + asmdef/manifest/README meta，**244 个 GUID 全部唯一**；`packages-lock.json` 已登记新包（这些文件尚未提交）。文档证据等级据此同步：编译 **E1（已实测）**，**运行时行为仍未验证**——编译通过 ≠ 打开窗口能列出 37 条示例并跑出正确输出，这是阶段 3.1 的验收缺口。另：Unity 工程已启用 **CoplayDev/unity-mcp**（`com.coplaydev.unity-mcp` 已在 manifest 与 PackageCache），但 **Claude Code 侧尚未注册服务器**（`claude mcp list` 为空），需在 Unity 的 MCP 窗口用 Auto-Setup 选择 Claude Code 完成注册，并重启会话使工具加载 |
| 2026-09-11 | 3.07 | **阶段 3.2 完成：语义图 IMGUI 渲染**。新增 `Editor/SampleVisualRenderer.cs`（~330 行）。**按实测数据修正设计口径**：`visualFrames` 覆盖 37/37、`nodes/edges/metrics` 仅 15/37，故改为**帧序列为主线、节点图为增量**（设计文档 §3.2 原假定节点图是主目标）；另核实 `laneLabels`/`highlightMetric`/`outputHints` 三个键在 C# 类型里不存在（Web 宿主直读 JSON 才用到）。**刻意不用 `Handles`**：连线用正交折线（`EditorGUI.DrawRect`）、箭头用递减宽度矩形拼——少一个无法离线验证的 API 面。布局纯拓扑分层（最长路径），不按 `node.kind`（29 处为空）。**本轮真正重要的产出是补上一个会静默漏报的验证工具**：写渲染器时误用 `string[].Count`（数组无 `Count` → CS0019），而原有离线探针**没报**——根因是不提供 Unity 程序集时编译器放弃周围绑定，可解析类型上的缺陷被吞。新建 `tools/unity-host-probe/`（最小 Unity 类型桩让绑定走完），**反向验证通过**（塞回 bug 精确报出 3 处 CS0019）。边界：桩不校验 Unity API 形状，不完整时报响亮误报（本次补 4 处）。验证：两个离线检查器 clean、构建 0 错误、Protocol 171/171、基线不变、`audit-versions` clean、门禁 10 步全绿（66.4s）。**阶段 3.1/3.2 的运行时行为仍待 Unity 侧实测** |
