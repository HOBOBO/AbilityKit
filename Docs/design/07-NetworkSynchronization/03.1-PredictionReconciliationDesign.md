...
| 通用状态同步 | **已实现**：`PredictionCoordinator` 提供预测、校验和服务器状态覆盖的抽象。 | 冲突时会先提取待重演输入，再清历史并重演；但输入历史会拍平命令并按每条命令推进一帧，同帧多命令会漂移。`Reset()`/`Dispose()` 通常也不会清掉已有非负帧。复杂项目必须约束输入粒度并自管历史生命周期。 |
...
| P0 | 通用 `PredictionCoordinator` 的同帧多命令重演会改变帧号 | `InputHistory.GetInputs` 拍平帧区间，`ReplayInputs` 按每条命令推进一帧；专项测试仅覆盖每帧一条命令。 | 修复为按帧分组重演并补多命令测试；完成前公开单命令约束，复杂业务使用 Host 的按帧输入数组。 |
