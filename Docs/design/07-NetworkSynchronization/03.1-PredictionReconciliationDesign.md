...
| 通用状态同步 | **已实现**：`PredictionCoordinator` 提供预测、校验和服务器状态覆盖的抽象。 | 冲突时先清空输入历史再调用重演；`Reset()` 以 `Frame.Invalid = -1` 裁剪更早快照，通常不会清掉已有非负帧。复杂项目不应把它当作完整重演和历史清理实现。 |
...
*文档版本：v2.1 | 最后更新：2026-08-09 | 状态：基于当前源码能力边界整理，待随 MOBA 表现对账桥接实现同步更新*
