...
> 基于 2026-08-03 框架源码审计。按严重度分 4 批。
> 最后更新: 2026-08-03 (第二轮: Batch 3.2 + Batch 4 部分)
...
AbilityKit 框架同时实现了帧同步（Lockstep FrameSync）和状态同步（Server-Authoritative StateSync），两者运行在复用的 `BattleLogicHostGrain` + StateSync 推送管道之上。
...
### 2.4 MobaSmoke 增加 FrameSync 模板测试
...
**验证命令**：
```bash
run_moba_multiprocess_smoke.ps1 --sync-template frame-sync-authority
```
...
- `Docs/design/10-EngineeringQuality/06-BetaStabilizationAndReleaseChecklist.md`
  - 第 35 行：更新已过时的 `ClientPredictionRunner`/`ClientPredictionReconciler` 废弃声明
  - 第 7 节末尾：新增 v0.1.0 Known Issues Backlog 表（11 项）
...
| 文件 | 内容 |
|------|------|
| `Unity/Packages/com.abilitykit.host.extension/Runtime/FrameSync/Design.md` | FrameSync 预测/回滚设计 |
| `.claude/skills/framesync-prediction-rollback/key_files.md` | 帧同步关键文件地图 |
| `.claude/skills/moba-demo/multiplayer_sync.md` | MOBA 多人同步能力矩阵 v0.1.0 |
| `.claude/skills/shooter-demo/snapshots_hash.md` | Shooter 三类快照 + AOI |
| `.claude/skills/shooter-demo/client_sync.md` | Shooter 三种客户端同步策略 |
| `Docs/design/10-EngineeringQuality/06-BetaStabilizationAndReleaseChecklist.md` | Beta 发布检查清单 + Known Issues Backlog |
